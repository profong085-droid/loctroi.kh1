        ...
        >
          {relatedProducts.map((product) => {
            return (
              <div 
                key={product.id}
                className="min-w-[240px] md:min-w-[280px] snap-start flex flex-col bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 overflow-hidden group hover:-translate-y-1"
              >
                <Link href={`/${locale}/product/${product.id}`} className="flex-1 flex flex-col">
                  <div className="relative h-48 sm:h-56 p-6 bg-linear-to-b from-slate-50 to-white flex items-center justify-center overflow-hidden border-b border-slate-50">
                  ...
                  <div className="p-5 flex-1 flex flex-col text-center items-center justify-center">
                    <h3 className="font-black text-slate-800 text-base md:text-lg line-clamp-2 mb-4 group-hover:text-primary-700 transition-colors">
                      {getLocalizedText(product.name, locale)}
                    </h3>
                    <div className="mt-auto pt-2 w-full">
                      <div className="inline-flex items-center gap-2 px-6 py-2 bg-primary-700 text-white rounded-full font-bold text-sm shadow-md group-hover:bg-primary-900 group-hover:shadow-lg transition-all w-full justify-center">
